#!/usr/bin/env node
// Thin-package inspection gate (doc 10 §11 LVG): asserts the packaged
// desktop app carries NO local-backend payload. Violations are the doc's
// closed list: .py files, Python/uv/uvicorn runtime, Brain backend,
// Go harness/service binary, local database, provider SDK/key material,
// Docker socket client, or internal (non-edge) service endpoint.
//
// Usage: node scripts/inspect-thin-package.mjs <release-dir-or-app-root>
...
if (!argRoot || !fs.existsSync(argRoot)) {
  console.error(
    'usage: inspect-thin-package.mjs <release dir, .app bundle, or unpacked dir>'
  );
  process.exit(2);
}
