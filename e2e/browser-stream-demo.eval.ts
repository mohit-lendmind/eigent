// Cloud demo driver (user request 2026-08-19): a browser-heavy research task
// on the cloud edge with the filmstrip open WHILE the run browses, so the
// recorded video shows frames streaming into the app live. The strip is
// reached the way a user reaches it (workforce expand → browser card) and the
// streaming claim is proven by sampling data-frame-count during the run: it
// must grow while the run is active, and the final count must equal the
// trajectory's published frame artifacts.
//
// Run: EIGENT_E2E_BOOTSTRAP=<bootstrap-cloud.json> \
//      npx playwright test --config e2e/eval.config.ts browser-stream-demo
// Output: EIGENT_EVAL_DIR (default ../cloud-demo-browser-stream).
...
const OUT_DIR =
  ...
  path.resolve(REPO_ROOT, '..', 'cloud-demo-browser-stream');
...
const RUN_TAG = `bs-demo-${Date.now().toString(36)}`;

// Single-newline runs vanish inside the contenteditable composer, so
// paragraphs join with blank lines only. Wikipedia is used because heavy
// production sites can crash the pod Chromium (halifax lesson).
const RESEARCH_BRIEF = [
  `[${RUN_TAG}] Browser research task — the HBM memory vendor landscape.`,
  'Use your browser tools to actually visit pages — do not rely on web_search alone. Visit these four Wikipedia articles in order, taking a screenshot on each: (1) High Bandwidth Memory, (2) SK Hynix, (3) Samsung Electronics, (4) Micron Technology. Navigate by opening https://en.wikipedia.org/wiki/High_Bandwidth_Memory first and following links or opening the article URLs directly.',
  'From what you read, write a comparison brief of the three HBM vendors (SK hynix, Samsung, Micron) to /workspace/hbm-vendors.md — founding, HQ, HBM involvement, one notable recent fact each — and publish it with the publish_artifact tool.',
  'Finish with a three-sentence summary of the HBM vendor landscape ending exactly with: ANSWER: HBM_BROWSE_DONE',
].join('\n\n');
const DONE_MARKER = 'HBM_BROWSE_DONE';
...
function trajectoryFacts(events: string[]): TrajectoryFacts {
  const facts: TrajectoryFacts = { terminal: null, frameArtifacts: 0 };
  for (const raw of events) {
    try {
      ...
      if (/^run_(completed|failed|canceled)$/.test(kind)) {
        facts.terminal = kind;
      }
      if (
...
test('cloud browser-stream demo: live filmstrip on kimi-k3', async () => {
  test.setTimeout(40 * 60_000);
  fs.mkdirSync(OUT_DIR, { recursive: true });

  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eigent-bsdemo-'));
  ...
  try {
    ...
    // Open the strip the way a user does, as soon as the run's browsing makes
    // it reachable: workforce mode → expand → the browser card.
    await expect(page.getByTestId('session-side-panel-header')).toHaveAttribute(
      'data-session-mode',
      'workforce',
      { timeout: 8 * 60_000 }
    );
    ...
    await expect(browserToggle).toBeVisible({ timeout: 5 * 60_000 });
    await expect(
      browserToggle,
      'the browser card is greyed out, so the strip is unreachable'
    ).toBeEnabled({ timeout: 2 * 60_000 });
    await browserToggle.click();

    const filmstrip = page.getByTestId('browser-filmstrip');
    await expect(filmstrip).toBeVisible({ timeout: 5 * 60_000 });
    await screenshot(page, '02-filmstrip-first');

    // The streaming proof: sample the strip while the run works. The frame
    // count must grow live, and the current image must be decoded pixels.
    const samples: { at_seconds: number; frame_count: number }[] = [];
    let shots = 0;
    let nextShot = 0;
    ...
    for (;;) {
      const count = Number(
        (await filmstrip.getAttribute('data-frame-count').catch(() => null)) ??
          '0'
      );
      samples.push({
        at_seconds: Math.round((Date.now() - tSubmit) / 1000),
        frame_count: count,
      });
      if (Date.now() >= nextShot && shots < 14) {
        shots += 1;
        nextShot = Date.now() + 90_000;
        await screenshot(
          page,
          `03-stream-${String(shots).padStart(2, '0')}-frames-${count}`
        );
      }
      ...
      if (Date.now() > deadline) {
        throw new Error('the browsing run never reached a terminal event');
      }
      await new Promise((resolve) => setTimeout(resolve, 15_000));
    }
    summary.terminal_event = terminal;
    summary.turn_seconds = Math.round((Date.now() - tSubmit) / 1000);
    summary.frame_count_samples = samples;
    summary.project_id = projectIdOf();

    const counts = samples.map((s) => s.frame_count);
    const grewLive = counts.some(
      (c, i) => i > 0 && c > counts[i - 1] && counts[i - 1] > 0
    );
    summary.stream_grew_live = grewLive;
    ...
    await expect
      ...
      .toBeGreaterThan(0);
    await screenshot(page, '04-filmstrip-final');
    const finalStripCount = Number(
      (await filmstrip.getAttribute('data-frame-count')) ?? '0'
    );
    summary.final_strip_frame_count = finalStripCount;

    const facts = trajectoryFacts(
      await fetchTrajectory(String(summary.project_id))
    );
    summary.trajectory_frame_artifacts = facts.frameArtifacts;

    // Close the overlay and read the settled answer.
    ...
    summary.presigned_requests = offEdge.filter((r) =>
      presigned.test(r.url)
    ).length;
    ...
    expect(
      ...
      'the run published no browser frames'
    ).toBeGreaterThan(0);
    expect(
      finalStripCount,
      "the strip's count disagrees with the trajectory"
    ).toBe(facts.frameArtifacts);
    expect(
      grewLive,
      'the frame count never grew while the run was active — nothing streamed'
    ).toBe(true);
    ...
    if (video) {
      ...
      if (recorded && fs.existsSync(recorded)) {
        const out = path.join(OUT_DIR, 'browser-stream-run.webm');
